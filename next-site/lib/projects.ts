...
export const PROJECTS: Record<ProjectId, ProjectMeta> = {
  ...
  'kubernetes': {
    ...
    difficulty: '🟡 中階',
    difficultyColor: 'text-yellow-400 bg-yellow-400/10 border-yellow-400/30',
    problemStatement: '你會用 kubectl 操作 cluster，但 control plane 4 個 process、kubelet、kube-proxy、CRI runtime、CNI 在背後到底做什麼？API server 收到 request 之後怎麼走進 etcd？scheduler 怎麼挑 node？kubelet 怎麼跟 containerd 對話？Pod IP 又是誰分配、誰寫 route、誰做 VXLAN 封裝？這個專案從架構切入，逐層拆到 reconcile loop、CRI、CNI 與 Linux datapath。',
    story: {
