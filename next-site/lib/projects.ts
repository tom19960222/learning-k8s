...
export const PROJECTS: Record<ProjectId, ProjectMeta> = {
  ...
  'kubevirt': {
    ...
    features: ['architecture', 'controllers', 'virt-handler-and-launcher', 'topology-spread-constraints', 'live-migration'],
    featureGroups: [
      { label: '從這裡開始', icon: '🚀', slugs: ['architecture'] },
    ...
    difficulty: '🔴 進階',
    difficultyColor: 'text-red-400 bg-red-400/10 border-red-400/30',
    problemStatement: 'KubeVirt 把整個 libvirt/QEMU stack 塞進 k8s — VM 變成 CRD，VM Pod 內跑 QEMU，live migration 由 controller 協調。但 5 個 virt-* process 各自做什麼？VM / VMI / VMIM 三個 CRD 怎麼接力？VM scheduling policy 怎麼傳到底層 Pod？live migration 是真的把 RAM 抄一份過去？這 5 頁從原始碼層拆。',
    story: {
      protagonist: '🧑‍💻 平台 SRE 你自己',
      ...
      scenes: [
        ...
        { step: 5, icon: '✈️', actor: '你', action: '讀 live-migration：memory 真的在搬', detail: 'precopy → iteration → cutover。dirty rate 不收斂時 fallback 到 postcopy。migration-proxy 把 libvirt 的 unix socket 走 TCP 跨 node。' },
      ],
