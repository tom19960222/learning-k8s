...
export const PROJECTS: Record<ProjectId, ProjectMeta> = {
  ...
  'kubevirt': {
    ...
    features: ['architecture', 'controllers', 'virt-handler-and-launcher', 'topology-spread-constraints', 'windows-vm-features', 'live-migration'],
    featureGroups: [
      { label: '從這裡開始', icon: '🚀', slugs: ['architecture'] },
      { label: '控制平面', icon: '🧠', slugs: ['controllers'] },
      { label: 'Node + Pod 內部', icon: '🖥️', slugs: ['virt-handler-and-launcher'] },
      { label: 'Scheduling', icon: '🧭', slugs: ['topology-spread-constraints'] },
      { label: 'Windows VM', icon: '🔐', slugs: ['windows-vm-features'] },
      { label: 'Live Migration', icon: '✈️', slugs: ['live-migration'] },
    ],
    usecases: [],
    difficulty: '🔴 進階',
    difficultyColor: 'text-red-400 bg-red-400/10 border-red-400/30',
    problemStatement: 'KubeVirt 把整個 libvirt/QEMU stack 塞進 k8s — VM 變成 CRD，VM Pod 內跑 QEMU，live migration 由 controller 協調。但 5 個 virt-* process 各自做什麼？VM / VMI / VMIM 三個 CRD 怎麼接力？VM scheduling policy 怎麼傳到底層 Pod？Windows VM 的 Hyper-V enlightenment、SMM、persistent vTPM 最後怎麼變成 libvirt/QEMU 設定？live migration 是真的把 RAM 抄一份過去？這 6 頁從原始碼層拆。',
    story: {
      protagonist: '🧑‍💻 平台 SRE 你自己',
      ...
      scenes: [
        ...
        { step: 4, icon: '🧭', actor: '你', action: '讀 topology-spread-constraints：VM policy 怎麼變 Pod scheduling', detail: 'VM template labels 與 VMI spec 一路傳到 virt-launcher Pod；真正計算 skew 的是 Kubernetes PodTopologySpread scheduler plugin。' },
        { step: 5, icon: '🔐', actor: '你', action: '讀 Windows VM features：Hyper-V flags 怎麼落到 QEMU', detail: 'hypervclock、Hyper-V enlightenment、SMM、persistent vTPM 不是同一層。KubeVirt converter 先翻成 libvirt XML，libvirt 再組出 QEMU hv-* CPU flags 與 swtpm backend state。' },
        { step: 6, icon: '✈️', actor: '你', action: '讀 live-migration：memory 真的在搬', detail: 'precopy → iteration → cutover。dirty rate 不收斂時 fallback 到 postcopy。migration-proxy 把 libvirt 的 unix socket 走 TCP 跨 node。' },
      ],
    ...
    learningPaths: {
      beginner: [
        ...
        { slug: 'topology-spread-constraints', note: '理解 VM template label 怎麼一路傳到 Pod scheduler' },
        { slug: 'windows-vm-features', note: '分清 hypervclock、Hyper-V enlightenment、SMM 與 persistent vTPM' },
      ...
      intermediate: [
        ...
        { slug: 'topology-spread-constraints', note: '拆 DoNotSchedule / ScheduleAnyway 的 scheduler path' },
        { slug: 'windows-vm-features', note: '追 KubeVirt converter 到 libvirt XML 與 QEMU hv-* flags' },
        { slug: 'virt-handler-and-launcher', note: 'virtwrap DomainManager 跟 libvirt 的接點' },
        { slug: 'live-migration', note: 'precopy / postcopy 的取捨' },
      ],
      advanced: [
        { slug: 'topology-spread-constraints', note: '從 PodTopologySpread plugin 追 PreFilter / Filter / Score' },
        { slug: 'windows-vm-features', note: '檢查 reenlightenment、TSC scaling、persistent TPM state 對 migration 的影響' },
  ...
  'ceph': {
    ...
    features: ['architecture', 'crush-and-placement', 'crush-straw-straw2', 'pool-pgnum-pgpnum', 'osd-and-bluestore', 'bluestore-deep-dive', 'mclock-osd-scheduler', 'recovery-throttle-runtime', 'osd-flapping', 'pg-health-states', 'mon-clock-skew-detection', 'cluster-time-skew-impact', 'clock-skew-magnitude-ladder', 'mon-quorum-loss-impact', 'osd-ok-to-stop', 'osd-safe-to-destroy', 'prometheus-alert-design', 'prometheus-alert-testing', 'prometheus-alert-real-lab-findings', 'prometheus-alert-real-cluster', 'mon-quorum-detection-blind-spot', 'slow-ops-and-bluestore-alerts', 'incident-bundle-runbook', 'rbd-and-csi', 'slow-ops-detection-mechanisms', 'slow-ops-fast-detection-lab', 'ceph-alert-policy-current', 'ceph-alert-validation-methodology', 'ceph-alert-real-lab-report', 'mon-quorum-loss-io-lab', 'clock-skew-failure-modes', 'recovery-throttle-and-mclock', 'rbd-datapath-and-csi', 'incident-bundle-operator-runbook', 'incident-bundle-validation-report'],
    featureGroups: [
      { label: '從這裡開始', icon: '🚀', slugs: ['architecture'] },
      { label: 'Placement 演算法', icon: '🧮', slugs: ['crush-and-placement', 'crush-straw-straw2', 'pool-pgnum-pgpnum'] },
      { label: 'OSD + Storage 引擎', icon: '💾', slugs: ['osd-and-bluestore', 'bluestore-deep-dive', 'mclock-osd-scheduler', 'recovery-throttle-runtime'] },
      { label: '故障偵測與恢復', icon: '🚨', slugs: ['osd-flapping', 'pg-health-states', 'mon-clock-skew-detection', 'cluster-time-skew-impact', 'clock-skew-magnitude-ladder', 'mon-quorum-loss-impact'] },
      { label: 'OSD 維運命令', icon: '🛠️', slugs: ['osd-ok-to-stop', 'osd-safe-to-destroy'] },
      { label: '監控與告警', icon: '📟', slugs: ['prometheus-alert-design', 'prometheus-alert-testing', 'prometheus-alert-real-lab-findings', 'prometheus-alert-real-cluster', 'mon-quorum-detection-blind-spot', 'slow-ops-and-bluestore-alerts', 'incident-bundle-runbook'] },
    ...
    learningPaths: {
      ...
      intermediate: [
        ...
        { slug: 'crush-straw-straw2', note: 'straw2 如何用 weighted draw 降低 reweight 時的資料移動' },
      ...
      advanced: [
        ...
        { slug: 'osd-flapping', note: 'flap pattern 對 mark-down / mark-out / subtree_limit / laggy EMA 的實際觸發路徑' },
        { slug: 'pg-health-states', note: 'PG / OSD 危險狀態如何變成 health check 與線上風險' },
        { slug: 'slow-ops-and-bluestore-alerts', note: 'slow ops 兩個 metric 的來源鏈，與 BlueStore 慢操作為何漏掉、alert 怎麼補' },
        { slug: 'crush-and-placement', note: 'CRUSHMap rule 如何從拓樸樹挑出 acting set' },
        { slug: 'crush-straw-straw2', note: 'straw / straw2 bucket algorithm 與 reweight movement 契約' },
  ...
  'systemd': {
    ...
    story: {
      ...
      scenes: [
        ...
        { step: 2, icon: '🌐', actor: '你', action: '讀 outage-and-maintenance：分清 offline、server 沉默、bad reply、root distance', detail: '沒有有效 NTP reply 時 timesyncd 不碰 CLOCK_REALTIME；漂移是硬體 ppm。server 壞掉時要看 retry、server selection、RootDistanceMaxSec，而不是只看 service active。' },
        { step: 3, icon: '📉', actor: '你', action: '讀 poll adaptation / tuning：max poll 決定多久拿到下一筆 sample', detail: 'manager_sample_spike_detection 的 |offset| > delay 是接受 sample，不是 spike；50ms+ 真偏移會被送進 kernel，PollIntervalMaxSec 控制的是最晚多久看見它。' },
        { step: 4, icon: '📈', actor: '你', action: '讀 Prometheus 監控：timex 是早期 proxy，ceph 是權威判決', detail: 'node_timex_offset_seconds 是 kernel residual offset，不是跨 node 量測；ceph_health_detail{name="MON_CLOCK_SKEW"} 才是跨 mon 50ms 的權威判決。' },
      ],
      outcome: '你知道 timesyncd 重啟那一刻 wall clock 可能瞬間前跳（不會後退），也知道斷網時它其實完全停手、server 失敗會經過不同 selection 路徑、poll max 決定 sample 新鮮度而不是保證 slew 收斂時間。對 ceph 偵測來說：timesyncd / node_exporter 是早期訊號，MON_CLOCK_SKEW 才是跨 node 權威判決。',
    },
    learningPaths: {
      beginner: [
        { slug: 'timesyncd-restart-impact', note: '從一個具體問題切入：service restart 對系統時間的影響' },
        { slug: 'timesyncd-log-anatomy', note: '先知道 journal 預設看得到什麼、看不到什麼' },
        { slug: 'timesyncd-outage-and-maintenance', note: '理解斷網與 server down 時其實是 free-run，不是 timesyncd 在補償' },
      ],
      intermediate: [
        ...
        { slug: 'timesyncd-poll-interval-adaptation', note: 'manager_adjust_poll 四條分支與 spike detection 順序' },
        { slug: 'timesyncd-low-max-poll-simulation', note: '把 PollIntervalMaxSec 壓太低會犧牲 adaptive poll 與 noise immunity' },
      ...
      advanced: [
        ...
        { slug: 'timesyncd-poll-tuning-from-metrics', note: '用實測 offset / jitter / root distance 反推 max poll 的平衡點' },
        { slug: 'timesyncd-drift-lab-experiments', note: '25 cells 失聯恢復矩陣與 PollIntervalMaxSec=256 生產級 soak 的 source 推導預測' },
        { slug: 'timesyncd-inference-vs-measurement', note: '純讀原始碼的推論搭 VM 實測後對帳：恢復時間從「幾分鐘」收斂成「十幾秒」、跳變偵測是 TFD 即時而非 poll-bound' },
        { slug: 'timesyncd-outage-and-maintenance', note: 'server selection、bad reply、RootDistanceMaxSec 的排障邊界' },
        { slug: 'timesyncd-prometheus-monitoring', note: '三層監控設計 + per-node residual offset 當跨 node 50ms 的早期 proxy' },
        { slug: 'node-exporter-timex-offset-source', note: '從原始碼追 node_timex_offset_seconds：timex collector → adjtimex(2) → kernel PLL；divisor 由 STA_NANO 決定，offset 是 residual 不是即時量測' },
      ],
    },
  },
  'rook': {
  ...
  'vm-storage-perf': {
    ...
    color: 'cyan',
    accentClass: 'border-cyan-500 text-cyan-400',
    features: ['linux-io-stack-primer', 'block-layer-blk-mq-primer', 'async-io-interfaces-primer', 'virtio-vring-primer', 'kvm-io-boundary-primer', 'kernel-async-and-net-primer', 'rbd-io-datapath', 'rbd-io-tuning-catalog', 'rbd-io-experiment-plan', 'rbd-io-production-tuning', 'rbd-io-research-report'],
    featureGroups: [
      ...
      { label: '效能調教', icon: '🎛️', slugs: ['rbd-io-tuning-catalog', 'rbd-io-experiment-plan', 'rbd-io-production-tuning', 'rbd-io-research-report'] },
    ],
    usecases: [],
    difficulty: '🔴 進階',
    difficultyColor: 'text-red-400 bg-red-400/10 border-red-400/30',
    ...
    learningPaths: {
      beginner: [
        { slug: 'linux-io-stack-primer', note: '看不懂 kernel 底層名詞？先讀這 6 篇底層基礎 primer 建立地基' },
