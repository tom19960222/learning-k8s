...
## E-09(b) 結果（inject 07:25:17Z / rollback 07:38:43Z）— tuned Drift 車道如預測 ~3min
- v2 `CephNodeClockVsPrometheusDrift`（0.05/for:2m）**T+2.9min firing**、正確指認 mon-1 ✓
- Ceph 這局 **T+57s** 就 WARN — round 1 剛結束、timecheck 暖狀態（冷啟動 4.6min vs
  暖 57s）→ 修正結論：Ceph 偵測延遲隨 round 相位浮動 1~7min；v2 tuned = 確定性
  ~3min 上界 + 不依賴 mon/mgr pipeline（H-021 情境仍活）— 互補而非零和
- daemon offset 因 freq 汙染偶竄 >50ms → Offset/Spread pending 進出、for 正確吸收零誤發
- 門檻定案依據：Drift warning 0.05s/for:2m、critical 0.5s/for:1m（雜訊 +5ms、10× 裕度）

## H-033 純 transient（k8s-1, 07:39Z）
- NTP 連線下 step +100ms → 真實 offset 回 <20ms 於 **12.7s**（watcher 立即 resync）
- timesync-status 事後顯示 Offset -100.244ms（修復量測的滯後快照）— snapshot 語意三證
- 判決：transient 快於任何 scrape+for 鏈 → keep_firing_for / kernel tripwire 是唯一
  能留下痕跡的機制（E-07 已示範 Unsync 路徑）

## E-10 剩餘情境：停 node-exporter（osd-0, 07:38:45Z）
- prediction: up{osd-0}=0 → v2 MetricsMissing 正確不 fire（需 up==1）；無任何
  target-down 規則存在 → **缺口：部署包需補標準 up==0 alert**（觀測確認中）

## E-08 soak 起點
- 07:39Z 起：tuned v2 規則 + 5 台 collector 全開的健康態即為 soak；已知例外 =
  osd-2 MetricsMissing（by design）+ baseline 規則的既知 FP（對照組保留）
- E-10(§6.3) 結果（07:38:45–07:50:07Z）：up{osd-0}=0 共 11 分鐘，全規則集零 alert
  提及 osd-0（MetricsMissing 正確不 fire — 前提 up==1）→ 缺口實錘 → 補第 17 條
  `CephNodeExporterDown`（up==0 for 5m）+ 對應 inhibition；node-exporter 已恢復

## E-08 soak 收官（07-23 07:39Z ~ 07-24 01:28Z，乾淨窗 17.8h）
- **v2 16 條 per-node 規則（排除 osd-2 已知例外）：零 firing、零 pending** —
  17.8h 內沒有任何規則接近過誤發（pending 都沒有）
- 對照組 baseline `Degraded`：健康 5 台連續 firing ~18h（樣本 6500-6700 @10s eval）
  至收官當下仍在叫 — 永久 FP 的完整量化
- 22h 窗的 v2 firing 樣本全數對應注入期（Drift=E-06/E-09、Unsync/Stalled/
  KernelErrorBound=E-07、TextfileScrapeError=E-02a）— 無一筆無法歸因
- 收官時：drift 全 fleet ≤6.6ms（雜訊帶內）、HEALTH_OK、quorum age 25h（全戰役
  零 election）、9 OSD up/in
- 閘門判定：17.8h < 原定「多日」，經濟因素（$75/天）提前收 — 生產上線首週仍建議
  接非 paging receiver 過渡（報告 §8 下一步 2 不變）
