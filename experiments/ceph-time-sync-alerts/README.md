...
| 回填表 | **已執行 2026-07-23（9/9 台，全同質）**：systemd **249.11**（Ubuntu 22.04）、kernel 6.8.0-1062-azure。NTP daemon＝**chrony active**（timesyncd inactive），唯一 source＝`PHC0`（`/dev/ptp0`=hyperv）→ **全 cluster 直接跟 Azure host 時鐘，零網路 NTP，天生共模**。`hv_utils` TimeSync IC 4.0 內建。**cephadm（Ceph 19.2.5）只部署 mon/mgr/osd/crash — 無任何監控堆疊**（無 node-exporter/Prometheus/Alertmanager、textfile 目錄不存在）→ 需自佈（見 §4.6 前置）。mon-0 有 admin keyring（`ceph orch` 可用）。timesyncd 無任何自訂 config。K8s 節點不在 ceph host 清單內 |
...
**E-00 後的部署前置（本 lab 實況）**：
1. `ceph orch apply` node-exporter（placement `*`，`extra_entrypoint_args` 加
   `--collector.textfile.directory=/etc/node-exporter` 與 `--collector.timex` — 後者能否
   蓋掉預設 `--no-collector.timex` 是 H-037 的實測點）+ prometheus/alertmanager
   （placement `cyshih-osd-2`）。
2. **信任域切分**：`cyshih-osd-2`（Prometheus host）與 `cyshih-k8s-0`（NTP 上游）**保持
   chrony→PHC**（Azure host 時鐘域）；其餘 5 台 ceph host + `cyshih-k8s-1`（L2）轉
   timesyncd→`10.0.1.5`（k8s-0 chrony 加 allow 當 LAN NTP server）、`PollIntervalMaxSec=256`
   （對齊生產）。如此 Drift 車道的兩端才真正跨信任域（codex r2 finding 3 的架構解）。
3. k8s-2 保持原樣（對照組）。
