...
| 層級 | 機器 | 用途 | 必要性 |
|---|---|---|---|
| L1 | 本機 macOS | script 單元測試（fake `timedatectl` PATH 覆蓋，bash 3.2 相容） | 必要，零成本 |
| L2 | 1× VM（Ubuntu 22.04 或與生產同版）| parser 邊界、metric 語意、timesyncd 行為 | 必要 |
| L3 | 3× VM（同上）+ 1× VM（NTP server + Prometheus + Alertmanager + node_exporter targets）| alert 全鏈路、spread / 對稱 skew / 偵測延遲 | 必要 |
| L4 | 既有 ceph lab（3 mon + 9 OSD，`.166` seed）或等效 | MON_CLOCK_SKEW 賽跑、cephadm textfile 路徑、observer 失效 | 必要（可用現有 lab）|

VM 規格：2 vCPU / 2GB RAM / 20GB disk 即可。L3 的 3 台 client VM 需互通、且能連 server VM
的 udp/123 與 Prometheus scrape port。**與生產同版 OS / systemd / NTP daemon 最重要**
（parser 行為隨 systemd 版本變動；E-00 會先盤點）。

PVE 注意事項（沿用 timesyncd lab 教訓）：實驗窗口停用 vzdump / snapshot / live migration、
停 `qemu-guest-agent`（guest-set-time 會改時鐘）、開跑前拍 snapshot 當最後回退手段。
時鐘會被大幅撥動（最大 ±5min）：**client VM 必須是乾淨實驗機**。
...
| 調整的參數 | PollIntervalMaxSec = 32 / 256 / 2048；固定注入 100ppm（`clock_inject.py`）；固定斷線 30min（iptables OUTPUT udp/123 DROP，只動這條）|
...
| 回填表 | （case／預期／promtool 實際／verdict）|
...
- 拆掉 `or on()` 混合條件、每條件獨立 alert 保留 instance label；spread 改 `max(o)-min(o)`；
  ...
  packet rate 窗口配合固定 `PollIntervalMaxSec=256`（timesyncd lab exp4 生產建議）或改用 last-sync-age
