...
| **v2 採集 script** | `v2/collect_time_sync.sh` + timer units | L1 測試 22/22 綠（`tests/parser/run-v2-parser-tests.sh`）|
| **v2 alert 規則（15 條）** | `v2/ceph-time-sync-alerts.yml` | promtool 13 組全綠（含健康基線零 FP）；`[draft-EXX]` 門檻待實驗定案 |
| v2 規則測試產生器 | `tests/promtool/gen-v2-rules-test.py` | 改情境改這支再重產，勿手改 test.yml |
| E-00 盤點腳本（唯讀）| `run/e00-inventory.sh` | 備妥未執行 |
