...
`env.sh` 至少要填 `SERVER_IP`（client 看得到 server 的那個 IP）與 `CLIENT_IFACE`（exp4 的 jitter 情境用 tc netem 套在這張網卡上；注意 exp4 啟動時一律檢查這個值，**即使跑的是不碰 tc 的情境（如 L3 的 inject-80ms 煙霧）也必須填**）。`NTP_PORT` 與 `RESULTS_DIR` 有合理預設可不動。`env.sh` 已被 `.gitignore` 擋掉，不會被 commit。
...
L3 不跑 `setup-client.sh`（那是 L4 指向真 server 用的），所以它裝的依賴要先補上——`preflight` 缺 `iptables` 或 `ping` 會直接 die：

```bash
apt-get update -qq && apt-get install -y -qq iputils-ping iproute2 iptables python3
```

```bash
ssh root@<CLIENT_IP>
cd /root/timesyncd
# env.sh 設 SERVER_IP=127.0.0.1（CLIENT_IFACE 也要填，exp4 啟動檢查需要）
...
- **時鐘玩壞了**：`cd /root/timesyncd && python3 lib/clock_inject.py reset` 把 tick/freq/offset 暫存器歸零，再 `systemctl restart systemd-timesyncd` 讓 timesyncd 對回真時；最後手段是 PVE snapshot 回滾整台 VM。
