...
- gray failure 實驗（E-32）對一台 OSD host 注入 50ms 網路延遲，驗證「OSD 活著但很慢」的情況。
...
- 重跑最可能被「三個 OSD 共用一顆 NVMe」污染的單 OSD 故障、mClock、scrub 與 backfill 實驗（E-02、E-19、E-22、E-30）。

世代 2 **沒有重跑 gray failure 實驗（E-32）**，所以不能說 ×40 的影響已跨世代重現。它在本報告的用途，是校正「IO 過重／backfill 到底有多少是共媒體放大」：
...
| Azure 世代 1 | 規劃 35 個：30 獨立執行、1 併入、4 裁定不做 | backfill、gray failure 與 packet loss 實驗（E-30、E-32、E-33） | 參數調教、KubeVirt failover 與容量故障留在原報告，不拿來支持本題 |
| Azure 世代 2 | 重跑單 OSD 故障、mClock、scrub 與 backfill 實驗（E-02、E-19、E-22、E-30） | backfill 結果直接進主文；其餘用來界定拓樸 caveat | 不宣稱世代 2 重跑了 gray failure／packet loss 實驗（E-32、E-33） |
...
Azure gray failure 實驗（E-32）對一台 OSD host 增加 50ms 網路延遲；OSD 全部活著、heartbeat 正常，`ceph -s` 全程 `HEALTH_OK`：
...
Azure packet loss 實驗（E-33）也推翻一個直覺：同一個低 RTT 環境注入 0.1%／0.5% 隨機丟包，已報的 client latency／throughput 指標最壞效果約 1.1×；TCP 快速重傳吸收了稀疏丟包。這不是說 packet loss 永遠安全，而是說在該實驗邊界內，**RTT latency 比少量 loss 更值得優先監控**。
...
Azure gray failure 實驗（E-32）已經證明寫入 ×40 時仍可 `HEALTH_OK`。Ceph health 主要描述 quorum、daemon、PG、容量與已知 health check；它不是 workload latency SLO。
...
| Azure gray failure 與 packet loss（E-32、E-33） | 4KiB randwrite qd8＋randread qd1 並行，1 秒粒度；分別注入 host RTT +50ms、隨機 loss 0.1%／0.5% | degraded 長窗 scenario；不是 n≥3 的效能 A/B |
| Azure auto-out 與 backfill（E-30） | 同一 degraded 負載；停單顆 OSD 並跨 600s auto-out；世代 2 down 751s、backfill 觀察約 150s | 兩個世代各一個長窗；跨世代交叉驗證，不是單變因 A/B |
...
| Azure gray failure（E-32） | 驗 health OK gray failure | host RTT +50ms | health OK 但 client 變慢 | ✅ 寫 1.73->69.6ms（×40）、讀 ×19 | client p99＋RTT 是必要訊號 |
| Azure packet loss（E-33） | 驗 loss 是否比 latency 更傷 | 低 RTT 下 loss 0.1%／0.5% | p99.9 應 >5× | ❌ 已報 client 指標最壞約 1.1× | 只在此低 RTT／稀疏 loss 邊界下先看 RTT |
| Azure 兩代 auto-out 與 backfill（E-30） | 驗共媒體 caveat | OSD down -> auto-out/backfill | 獨占媒體應減少持續爭用 | ✅ mean ×24 -> ×14；世代 2 median ×1.0 | 結果與拓樸影響一致，但非單變因因果證明 |
...
Azure gray failure 實驗（E-32）提出的起手式是 client p99 >自身 baseline 3 倍、持續 5 分鐘；這是**待生產 shadow 驗證的起始值**，不是已完成 alert-lab 驗證的通用門檻。
...
- Grafana 同頁放 client p99、單 OSD 暫態、同 node 聚集、commit/apply latency、RTT、recovery 與 device busy。
...
| 立即 | client p99 dashboard／alert | health OK 的 gray failure | Azure gray failure 實驗（E-32）寫入 ×40 | 中；需 workload metrics |
| 立即 | OSD-node-device inventory | 告警無法對到實體硬體 | 多 OSD 共用元件判讀需求 | 低至中 |
| 近期 | 同頁呈現 RTT／recovery／device | SSD 與過載誤歸因 | 無故障高負載、backfill、gray failure 實驗（E-00、E-30、E-32） | 中 |
...
2. **Azure gray failure 實驗（E-32）只在世代 1 執行**：這個機制很重要，但 ×40 沒有在世代 2 重跑。
3. **Azure 世代 2 資料量小且不只改一個變因**：每 OSD 約 8GiB，backfill 約 150 秒；VM 規格、OSD 數與 OSD 的 mClock 效能校準值也不同，跨世代不是乾淨 A/B。
...
10. **alert 成功不等於 recovery 證據完美**：latency-outlier 實驗（S11）的正向 firing 與 routing 斷言通過，但 BlueStore 24h latch 讓同一 run 內沒有 `HEALTH_OK`；健康恢復證據來自下一場 run 的 ready gate。
...
- [Azure 實驗 hypothesis backlog](../kubevirt-rbd-tuning/HYPOTHESES.md)
