...
| E-09 layout（補跑） | 07-08 00:19–02:0x | 手動 map 三顆 layout image 逐建測刪 | `results/exp9-layout/rerun-20260708-001949/`（主 checkout 已複製） | 全帶內 ➖——striping 未幫 seq（兩顆 OSD + 1G 封頂結構限制）；首跑 PVE volume 命名失敗已修（10b3e40） |
| SSD 調查 | 07-08 02:5x | read-only（ceph device ls/get-health-metrics） | snapshot §SSD 調查 | **osd.0 = MX300 磨損 90%（erase 1360/~1500）**、osd.8 = OCZ ARC100，皆無 PLP——寫入天花板根因確立 |
| 資源清理 | 07-08 02:43 | cleanup.sh + snippet/pub 移除 | — | VM 1031-1039 範圍 0、ioperf pool 0 image、storage.cfg 無 ioperf-krbd、ds216j snippet 已刪；叢集回基線 HEALTH_WARN（既有 osd.0 slow-ops） |
