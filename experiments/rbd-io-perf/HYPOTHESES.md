...
- 環境（2026-07-07 手動 pre-flight 已盤點，詳見 `preflight-snapshot-2026-07-07.md`）：node pve3（192.168.16.7，ioperf 帳號 + sudo）；PVE 9.0.11 / pve-qemu 10.1.2 / kernel 6.14.11-4-pve / ceph 19.2.3-pve2（與 pinned 同版）；測試 pool `ioperf`（ssd-only、size2/min_size2，僅 osd.0+osd.8 兩顆 SSD，**osd.0 有基線 BlueStore slow ops WARN**）；hyperconverged（pve3 跑 osd.8）；**pve3 ceph public 介面 100Mb/s**（雙峰 confound，見 H-034）；VMID 1031–1039；guest 網 vmbr1（192.168.18.0/24 DHCP）；磁碟預算 boot 10G + data 16G 逐顆建刪（SSD class 峰值 ~42%）。krbd 先例：cluster 已有 `krbd 1` 的 `ceph-ssd` storage。
- Tiers available:
...
- Status: confirmed（E-03 回答：稅整體被 ceph 側淹沒）
- Tier: T3
...
- Status: ~~proposed~~ violated（E-14：全程 dmesg 零筆 bad crc——Linux guest + O_DIRECT fio 不觸發；條件預測的 null 分支成立，Windows guest 情境未測）
- Tier: T3
...
- Status: confirmed（E-03 n=5：qd1 讀 CoV 23.4%、qd1 p99 CoV 22.9% >> 5%；qd32/seq 2.4–11%——單次比較確定不可信）
- Tier: T3
...
- Status: confirmed（E-02 已跑，天花板成立且改寫後續解讀）
- Tier: T3
- Origin: negative-space
- Prediction: 高 QD 下兩 engine 差異落噪音帶內；qd1 io_uring 略優。
- Evidence: bundle `results/exp0/20260707-032324/`（n=3）。**寫側天花板災難級**：rw 4k qd1 = 33 IOPS（~30ms/op）、qd32 = 746 IOPS、seqwrite 17.7 MB/s——與 osd.0 slow-ops WARN + 消費級 SSD 同步寫特性一致；讀側 qd1 1126 IOPS / qd32 19.9k IOPS。seqread 202 MB/s > 1G 線速 112 MB/s → 本機 primary（osd.8）不走網路（H-034 雙峰直接得證）。engine 對照機器判定：高 QD 讀 violated（libaio 快 25.6% > 帶 23%）、qd1 indistinguishable（帶 51%）。guardrail 全程未觸發。
- Artifacts: 頁面 E-02 回填 + 解讀 Callout（寫側旋鈕預期大面積 indistinguishable）。
...
- Notes: 與 H-001 同型的「baseline 幻覺」。**T3 已驗（2026-07-07 E-03）**：PVE 預設 VM（4 vCPU、什麼都沒設）guest `/sys/block/vdb/mq/` = 4 個 hw queue——多 queue 就是預設。libvirt 中間層已查證（2026-07-07）：libvirt/src/qemu/qemu_command.c:1692 `"p:num-queues", disk->queues`——`p:` 修飾詞在值為 0（domain XML 未設 queues）時不輸出屬性，QEMU AUTO 預設直通；整條鏈（KubeVirt converter 只在 blockMultiQueue 時設 Queues → libvirt 未設不輸出 → QEMU AUTO=vCPU 數）T1 完整。
...
- Status: confirmed（E-04：krbd seqwrite +17.9% 超帶；讀側 krbd 均值 +7~17% 且 CoV 5-6% vs librbd 20-22%——krbd 不輸且顯著更穩；bundle results/exp-axis/20260707-092652）
- Tier: T3
...
- Status: confirmed（T1 + T3：E-05 writeback rw-qd1 +6,871%/qd32 +219%、writethrough qd32 −68.7%——rbd_cache 機制實測命中；bundle results/exp1-cache-*/）
- Tier: T1
...
- Status: confirmed（krbd usable，雙軸計畫成立）
- Tier: T3
- Origin: framing-dialog（使用者：「你可能要先測試能不能用 krbd」）
- Prediction: 現代 kernel（6.14）對 PVE 預設 image features 應可 map。
- Evidence: `run/krbd-check.sh` 2026-07-07 03:22 執行——關卡 1（map + O_DIRECT 讀寫）過；關卡 2 features = layering, exclusive-lock, object-map, fast-diff, deep-flatten（kernel 6.14 全支援）；關卡 3（`pvesm add rbd ioperf-krbd --krbd 1`）過。輸出 `krbd: usable`。
- Artifacts: 頁面 E-01 回填。
...
- Status: confirmed（E-03 cmdline 實測：cache=none＝drive "direct":true、device write-cache=on、無 iothread；librbd driver 無 aio 欄位——aio 是 file-posix/krbd 軸專屬旋鈕，E-06 因此改 krbd 軸）
- Tier: T3
...
### H-034: pve3 的 ceph public 介面協商在 100Mb/s，從 pve3 出發、primary 在遠端 osd.0 的 PG 其 client IO 封頂 ~11 MB/s，所有延遲/吞吐分佈呈雙峰（local osd.8 快路徑 vs 遠端 osd.0 慢路徑），seq workload 的量測主要反映這條鏈路而非任何旋鈕
- Status: proposed
- Tier: T3
- Origin: pre-flight 盤點（2026-07-07，read-only 發現）
- Prediction:（E-02 時具體化）host 層 seqwrite/seqread 1M 的 BW 顯著低於 SSD 能力且對 primary 位置敏感；4k 延遲直方圖雙峰。
- Notes: 查證結果（2026-07-07）：enp5s0 = Realtek RTL8111（1G 卡），本卡與 switch 兩側都 advertise 1000baseT 卻協商 100Mb → 線材/埠問題，換線可回 1G。且 ceph `public_network` 就是 management 網段——client IO 結構性走這張卡，10G 只服務 OSD 複寫與 guest 網。使用者修線後重驗 E-00；即使回到 1G，高 QD 4k / seq 仍可能觸頂 ~112 MB/s（E-02 量化），假設主體不變、封頂值從 11 → 112 MB/s。
...
## Gate 3 執行記錄（2026-07-08）

使用者裁示：**批次 2 全做**、follow-up（E-06 krbd 軸 aio / E-15 手動 / krbd 讀側加輪）不做、SSD 調查授權執行、資源清理完成、ioperf 帳號清理 script 交付使用者。

- 批次 2 已落地（commit 見 git log）：catalog 新增 rxbounce 條目 + impact 表列、queue_depth 補 per-hctx 語意（H-011 重推：`BLK_MQ_F_SHOULD_MERGE`、無 shared-tag、容量 = depth × CPU 數，rbd.c L4958-4963）、librbd Callout 補 QEMU cache→rbd_cache 互動（H-028）、blockMultiQueue 補 E-08 實測註腳；experiment-plan 新增「結論」節（有效改動排序 + 天花板方法論 + SSD 根因）。
- SSD 根因（read-only 實查）：osd.0 = Crucial MX300 磨損 90%、osd.8 = OCZ ARC100，皆無 PLP——E-02 天花板與 slow-ops WARN 根因確立（H-034/E-02 的環境解讀收斂）。
- 清理：VM/image/storage id/snippet/pub 全清，叢集回基線。fio 套件保留（使用者授權安裝；移除指令在帳號清理 script 中列為選項）。
