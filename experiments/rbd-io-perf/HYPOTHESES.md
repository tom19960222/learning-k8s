...
- Notes: 與 H-001 同型的「baseline 幻覺」。T3 驗證：PVE 預設 VM 的 guest `ls /sys/block/vda/mq/` 應已見多個 hw queue。KubeVirt 軸還需查證 libvirt 是否會在 queues 未設時主動填 1（目前無證據）——批次 1 頁面修正前先做這個 T1 查證。
