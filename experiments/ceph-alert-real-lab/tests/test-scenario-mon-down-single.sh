...
if [[ "\$*" == *"exec prometheus-0 -- wget -qO- http://127.0.0.1:9090/api/v1/alerts"* ]]; then
  printf '%s\n' '{"data":{"alerts":[{"labels":{"alertname":"CephMonDownScoped","ceph_daemon":"mon.ceph-lab-mon-03","hostname":"ceph-lab-mon-03"},"state":"firing"}]}}'
  exit 0
fi
...
if [[ "\$*" == *"logs deploy/alert-sink"* ]]; then
  printf '%s\n' '{"receiver":"pager","alertname":"CephMonDownScoped","labels":{"ceph_daemon":"mon.ceph-lab-mon-03"}}'
  if grep -q 'systemctl stop ceph-' "$trace_file"; then
    printf '%s\n' '{"receiver":"pager","alertname":"CephMonDownScoped","labels":{"ceph_daemon":"mon.ceph-lab-mon-03","fresh":"true"}}'
  fi
  exit 0
fi
...
case "\$command" in
  *"ceph health detail"*)
    printf 'HEALTH_WARN 1 mons down, quorum reduced (MON_DOWN)\n'
    exit 0
    ;;
  *"ceph -s"*)
    printf 'HEALTH_OK\n'
...
find "$ROOT/results" -maxdepth 1 -type d -name 'mon-down-single-*' | sort >"$before_dirs_file"

set +e
PATH="$fake_bin_dir:$PATH" bash "$ROOT/run/scenario-mon-down-single.sh" >"$stdout_file" 2>"$stderr_file"
rc=$?
set -e

find "$ROOT/results" -maxdepth 1 -type d -name 'mon-down-single-*' | sort >"$after_dirs_file"

if [[ "$rc" -eq 0 ]]; then
  fail "scenario-mon-down-single should require destructive ack"
fi
[[ "$rc" -eq 2 ]] || fail "expected exit 2 without destructive ack, got $rc"
grep -Fq -- 'mon-down-single requires --yes-really-inject' "$stderr_file" || fail "missing destructive ack error"
...
set +e
PATH="$fake_bin_dir:$PATH" bash "$ROOT/run/scenario-mon-down-single.sh" --yes-really-inject >"$live_stdout_file" 2>"$live_stderr_file"
rc=$?
set -e
...
grep -Eq '^result: .*/results/mon-down-single-[^/]+$' "$live_stdout_file" || fail "missing result line on success stdout"
...
stop_line="$(grep -n '^ssh:sudo systemctl stop ceph-.*@mon\.ceph-lab-mon-03\.service$' "$live_trace_file" | head -1 | cut -d: -f1)"
start_line="$(grep -n '^ssh:sudo systemctl start ceph-.*@mon\.ceph-lab-mon-03\.service$' "$live_trace_file" | head -1 | cut -d: -f1)"

[[ -n "$stop_line" && -n "$start_line" ]] || fail "missing trace lines for ordering checks"
...
# the not-firing check for CephMonQuorumLost actually ran (a vacuous/never-called assertion would
# also pass the "not firing" check but would leave no evidence file behind).
result_dir="$(find "$ROOT/results" -maxdepth 1 -type d -name 'mon-down-single-*' | sort | tail -1)"
[[ -f "$result_dir/prometheus-alerts-CephMonQuorumLost-none.json" ]] || fail "missing negative-assertion evidence file for CephMonQuorumLost"

ok "mon-down-single destructive ack guard, injection sequence, and rollback ordering"
