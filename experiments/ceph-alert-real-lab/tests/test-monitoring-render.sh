...
grep -q 'alert: "CephOSDDown"' "$out" || fail "CephOSDDown mixin rule missing (default-mixin embed)"
grep -q 'inhibit_rules' "$out" || fail "inhibit_rules block missing"
grep -q 'alertname="CephMonQuorumLost"' "$out" || fail "inhibit source matcher missing"
grep -q 'alertname="CephMonDownScoped"' "$out" || fail "inhibit target matcher missing"
