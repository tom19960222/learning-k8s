...
[[ -n "$discover_line" && -n "$arm_line" && -n "$add_line" && -n "$del_line" && -n "$pkill_line" ]] || fail "missing trace lines for ordering checks"
(( arm_line > discover_line )) || fail "iface discovery happened after arming the auto-revert sleeper"
(( add_line > arm_line )) || fail "auto-revert sleeper was armed after the netem qdisc was applied, not before"
