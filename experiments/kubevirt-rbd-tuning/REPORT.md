...
> **最重要修正：E-30 頭牌的「backfill 隨機讀 ×24」在 clean 拓樸塌成偶發亞秒尖峰——×24 是共媒體病理，不是 backfill 本身。**
...
| 實驗 | 世代 1（共媒體）結論 | 世代 2（clean 獨占）結果 | 判定 |
|---|---|---|---|
| **E-02** host ceiling | 稅集中高並行隨機讀；寫入/循序幾乎免費（絕對天花板受共媒體壓低） | rr-qd32 **37.1k**、rw-qd32 **20.9k** IOPS；sr **3,111**、sw **1,168** MB/s。虛擬化稅仍集中在 rr-qd32（−22.2%）、sr-1m（−16.9%），寫/低 QD 幾乎免費（−5～+3%） | **機制維持、天花板刷新為真 NVMe 量級**（寫側不再是災難） |
| **E-19** queue_depth | 高並行 +10~21%、qd1 零代價；共媒體使效果解讀力弱 | qd256 vs qd64：rr-qd32x4 **+9.0%**、rw-qd32x4 **+28.0%**，且 **p99 同步改善**；qd1 ±1% 帶內 | **confirmed 且放大**：clean 下寫側增益 +28%——**共媒體壓低 queue_depth 效果的直接反證** |
| **E-22** shards 8→16 | 效果帶內（8 已足）；共媒體疑「蓋掉 shard 效果」 | 全帶內（rr-qd1 −3.1%、rr-qd32x4 −2.1%、rw-qd32x4 −1.8%）；rolling restart 6 OSD client 讀峰值 1.2–1.9s | **confirmed**：「8 已足」**非共媒體假象**，clean 隔離下 16 仍無增益；rolling=秒級 degraded 事件不變 |
| **E-30** 單 OSD down→backfill | down 無感；**auto-out 後 backfill 隨機讀 ×24**（頭牌） | down median 0.90ms 無感；**auto-out/backfill 相 client 讀 median 仍 0.90ms（×1.0）**，只剩單發 ~1s 尖峰 | **頭牌污染塌陷**：×24 是共媒體病理（recovery 與 client 搶同碟），clean 下塌成偶發亞秒尖峰 |

### 9.2 核心教訓
- **共媒體（多 OSD 共享一顆實體碟）會系統性放大「recovery/rebalance 搶 client IO」類的倍率**——E-30 的 ×24 是最極端案例，分離媒體後直接塌掉。
- **反過來，queue_depth 這種「靠並行填滿獨立媒體 queue」的增益，被共媒體壓低**——分離後才顯出真實 +28%。
- **機制級排序完全不變**：queue_depth=256 有用且 p99 更好、shards=8 已足、虛擬化稅集中高並行讀、writeback 危險（未重跑但機制不受媒體拓樸影響）。→ §6 參數建議全數維持。
- **一句話**：**絕對倍率看媒體拓樸，機制與相對排序看設計**。生產若採「每 OSD 獨占 NVMe」，本節數字比世代 1 更接近真實體感；若被迫共媒體，要預期 recovery 類事件的 client 懲罰被放大。

### 9.3 侷限
- E-30 clean 下每 OSD 資料量小（~8G）、backfill 快、尖峰窗短；TB 級/OSD 的生產 backfill 歷時更久 → 尖峰**機會**更多但機制（dedicated media → median 不受污染）可移植。
- E-19 生效驗證因 NSG 擋 worker internal IP 未跑 host config_info，改以行為級 A/B 差異證明（qd64≠qd256）；絕對倍率含 mclock outlier 噪音。
- writeback/CPU limit/failover 等機制級結論本次未重跑（與媒體拓樸無關，無重跑必要）。
