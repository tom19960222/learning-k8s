...
> **⚠ 2026-07-13 世代 2 重跑更新**：本輪（世代 1）跑在 Azure fallback＝**1 顆 NVMe 用 LVM 切 3 OSD、3 daemon 共媒體**。
> 後續把環境 redeploy 成 **6 OSD、每台 2 顆實體 NVMe 各 1 OSD（每 OSD 獨占媒體）**，重跑四個受「共媒體」污染的實驗（E-02/E-19/E-22/E-30）。
> **主要修正：E-30 頭牌的「backfill 隨機讀 ×24（均值、持續性）」在 clean 拓樸、本環境小資料量（~8G/OSD）下降為均值 ×14、且 median 不動（×1.0）——從「整秒窗持續慢」變「零星亞秒尖峰」；拓樸影響集中在 backfill（E-30）與高並行寫（E-19 +20.8%→+28.0%），E-02 天花板與 E-22 幾乎沒變。**
> 完整結果見 **§9** 與 `EVIDENCE-SUMMARY-2026-07-13.md`。凡本報告標「數值級」「本環境單 NVMe 切 3 OSD 放大倍率」處，均以 §9 為準。
...
| 編號 | 注入什麼 | 為什麼 | 預期 | 結果 |
|---|---|---|---|---|
| E-30 | 停 1 顆 OSD（乾淨 down），等自動 out、再回歸 | 最常見故障的完整生命週期 | 尖峰只在 peering 秒級窗 | ✅+細化：down 當下近乎無感；**真正傷害在 600 秒後 auto-out 觸發的 backfill——隨機讀 ×24（此為世代1共媒體均值；clean 拓樸修正為均值 ×14、median ×1.0，見 §9）** |
...
- **結果**：✅ 加上關鍵細化——分相位量測：down 當下 peering 尖峰僅 3.8ms；**down 但未 out 的 580 秒完全=baseline**；auto-out 後 backfill 相**隨機讀 mean ×24（0.96→22.7ms）、出現整秒級尖峰**（世代2 clean 拓樸下降為 mean ×14、median ×1.0，見 §9）；回歸後 recovery 相寫入 max 309ms；之後全自癒。
...
- **數值級結論不可直接搬運**：本環境 OSD 是單 NVMe 切 3（同碟效應放大倍率，如 E-30 的 ×24——世代2 分離媒體後降為 mean ×14、median ×1.0，見 §9）、nested 虛擬化。可搬的是機制、相對排序、以及完整的重測 harness（`RUNBOOK.md` 照抄可跑、`tools/fio_stats.py` 機器判 verdict）。
...
## 9. 世代 2 重跑：clean 6-OSD（每 OSD 獨占 NVMe）— 2026-07-13

**動機**：§1/§8 反覆標註「本環境單 NVMe 切 3 OSD 會放大某些倍率」。使用者把 Azure 叢集 redeploy 成 **L16s_v3、3 OSD host × 2 顆實體 NVMe 各 1 OSD = 6 OSD**（每 OSD 獨占 1 顆 1.74 TiB NVMe），重跑四個最受「共媒體」污染的實驗，隔離出「哪些結論是共媒體假象、哪些是真機制」。pool 仍 `kubevirt` size3/min_size2/pg64、ceph 19.2.4、kubevirt v1.5.0、kernel 6.8——**只有 OSD↔媒體拓樸變了**，可乾淨對照。harness＝`tools/env-gen2.sh` + `e19/e22/e30-gen2.sh`；逐數字見 `EVIDENCE-SUMMARY-2026-07-13.md`。

### 9.0 新環境的一個真實落陷（記給生產）
cephadm 在 OSD 建立時跑的 mclock 校準 `osd_mclock_max_capacity_iops_ssd` 在**同型 NVMe 上嚴重不均**：osd.0/1/3/5 ≈ 6.5k，但 osd.2 = 31.9k、osd.4 = 68.2k（同硬體 5–10× 差，cold-cache bench fluke）。**E-02 aggregate 讀 qd32 實測 37.1k ≈ 6×6.5k=39k → 證實 ~6.5k 才是真 per-OSD 天花板**，兩顆 outlier 是 over-measured。生產含義：換碟/重建 OSD 後值得 `ceph config dump | grep mclock` 抽查，outlier 會讓 mclock QoS 配額在 OSD 間不對稱。

### 9.1 逐實驗（五欄：調什麼／預期／世代 1 數字／世代 2 數字／判定）

> 判定欄回答的是**「這條結論被共媒體污染了嗎」**，不是「參數該不該調」（後者見 §6，四條結論全數維持）。
> 效果一律 before→after＋倍率；每條判定句自帶適用條件。

#### E-02 host 層天花板／虛擬化稅
- **調什麼**：不調參數；worker 直接 `rbd map` 繞過 VM，量單 VM 的 host 天花板與虛擬化稅。
- **預期（跑前寫死）**：clean 專用 NVMe 的天花板應**明顯高於**共媒體。
- **世代 1（共媒體）**：host rr-qd32 = **36,602** IOPS（= 聚合天花板 9×6000=54k 的 **68%**）；guest 26,771（= 54k 的 **50%**，單 VM qd32 打不滿聚合，虛擬化稅 +36.7%）。
- **世代 2（clean 獨占）**：host rr-qd32 = **37,107** IOPS（= 聚合天花板 6×6,500≈39k 的 **95%，近乎打滿**）；guest 28,859（= 39k 的 74%，虛擬化稅 +28.6%）；rw-qd32 = 20,892、sw-1m = 1,168 MB/s；3 輪 CoV 0.2–2.2%。
- **判定：預期被推翻（❌）— E-02 是四個實驗裡受拓樸影響最小的一個。** 交付的 host 讀天花板幾乎沒動（36,602→37,107，**+1.4%**），但**兩代的成因不同**：世代 1 是 68%（聚合有 54k、單 client qd32 的並行度用不完，client-concurrency 受限）、世代 2 是 95%（聚合只剩 39k、被真天花板卡住）。**交付數字相同、共媒體卻沒現形**，是因為**穩態單 client 讀不製造「多來源搶同碟」的爭用**——共媒體的懲罰要有爭用（backfill/recovery、多 VM 齊發）才會咬人，這種天花板測試造不出來。（先前把「17.7 MB/s→1,168 MB/s」當佐證是錯的——17.7 MB/s 來自更早的 PVE 消費級 SSD 研究，非本系列世代 1，已撤回。）

#### E-19 krbd queue_depth（qd64 vs qd256，基準 qd128）
- **調什麼**：krbd `queue_depth` 64 / 256（建置期定死：StorageClass 建立時設）。
- **預期（跑前寫死）**：clean 每 OSD 獨占 NVMe 下，高並行的 queue_depth 效果**更能顯現**（無同碟鄰居搶 IOPS）。
- **世代 1 → 世代 2（qd64→qd256）**：
  | pattern | 世代 1 | 世代 2 |
  |---|---|---|
  | rr-qd32×4（讀） | 49,086→54,129（**+10.3%**、p99 −14.8%） | 49,799→54,304（**+9.0%**、p99 −15.2%） |
  | rw-qd32×4（寫） | 19,798→23,914（**+20.8%**、p99 −11.4%） | 28,759→36,817（**+28.0%**、p99 −29.5%） |
  | rr-qd1 / rw-qd1 | −1.1% / +0.2%（帶內） | −1.1% / +1.2%（帶內） |
- **判定：confirmed，但拓樸只影響寫側。** **讀側效果新舊幾乎相同**（+10.3%→+9.0%）；差別在寫側——絕對 IOPS 從世代 1 的 19,798（qd64）墊高到世代 2 的 28,759，加成也從 +20.8% 擴到 +28.0%。**「共媒體壓低 queue_depth 效果」只成立在寫側**（高並行寫要填滿獨立媒體佇列時），讀側不成立。qd1 兩代都零代價。

#### E-22 osd_op_num_shards_ssd（8 vs 16）
- **調什麼**：`osd_op_num_shards_ssd` 8 / 16（rolling restart 才生效）。
- **預期（跑前寫死）**：clean per-OSD 隔離下 shard 效果**才量得準**，可能仍帶內或高並行小幅得利。
- **世代 1 → 世代 2（8→16 效果）**：世代 1 rr-qd32×4 +2.2%、rw-qd32×4 +0.1%（全帶內）；世代 2 rr-qd32×4 −2.1%、rw-qd32×4 −1.8%（全帶內）。
- **rolling restart 的 client 讀代價**：世代 1（9 OSD）to16 mean 39ms、p99 1,360ms、max 2,553ms；世代 2（6 OSD）to16 mean 63ms、峰值 1,889ms（1s 平均）。
- **判定：confirmed，拓樸幾乎無影響。** shards=16 對 8 兩代都零收益 → **「8 已足」不是共媒體假象**（這是本實驗要澄清的疑點，澄清了：clean 隔離下 16 仍無增益）。rolling restart 造成的秒級 client 讀尖峰兩代同量級（~1.4–1.9s）→「改 startup 參數＝一次 degraded 事件」不變。

#### E-30 單 OSD 乾淨 down → auto-out → backfill（頭牌）
- **調什麼**：不調參數；stop osd.3、撐過 600s auto-out 觸發 backfill，量 client 讀（隨機讀 qd1）延遲。
- **預期（跑前寫死）**：down 仍近乎無感；auto-out backfill 的 client 讀懲罰**應大幅縮小**（recovery 讀走 peer OSD 自己的 NVMe，不再與 client 讀擠同碟）。
- **世代 1（共媒體）**：down 未 out 階段 = baseline（0.95ms）；**auto-out/backfill 階段 rr-qd1 mean 22.73ms（×24 vs 基準 0.96ms）、max 1,011ms，且為「整秒窗」持續性劣化**。
- **世代 2（clean 獨占）**：down 未 out 階段 mean 0.99ms（=baseline）；**auto-out/backfill 階段 rr-qd1 mean 13.04ms（×14 vs 基準 0.92ms）、max 1,007ms，但 median 仍 0.90ms（×1.0）**。
- **判定：confirmed，且是四個實驗裡拓樸影響最大的一個——但要精確講：不是「歸零」，是「從持續變零星」。** 均值懲罰約砍半（×24→×14），最大尖峰兩代都 ~1s；**關鍵差異在 median：世代 2 為 0.90ms（×1.0）不動，代表典型請求正常、只有少數幾秒被單發尖峰拉高均值**（149 個 1s 樣本裡約 4 秒是尖峰）。世代 1 是「整秒窗」持續慢、世代 2 是「零星亞秒尖峰」。**成立條件**：本環境每 OSD 實資料量小（VM 16G×3 replica÷6 OSD≈8G/OSD）、backfill 窗僅約 150s——TB 級/OSD 的生產 backfill 歷時更久 → 尖峰**次數**會更多，唯「dedicated media→recovery 不與 client 搶同碟→median 不受持續污染」的機制可移植。

### 9.2 綜合：拓樸到底影響了什麼

| 實驗 | 拓樸影響 | 原因 |
|---|---|---|
| **E-30** backfill | **大**（×24 mean→×14 mean，median ×1.0 不動、持續→零星） | recovery IO 要跟 client 搶同碟，共媒體時全命中、分離後只剩零星尖峰 |
| **E-19** 寫側 | **中**（+20.8%→+28.0%，絕對 IOPS 24k→37k） | 高並行寫要填滿獨立媒體佇列，分離後基底更高、加成更大 |
| **E-19** 讀側 / **E-22** / **E-02** | **幾乎無**（讀 +10.3%→+9.0%、shards 兩代帶內、讀天花板 36.6k→37.1k） | 穩態單 client / 參數本來就夠 → 不製造「多來源搶同碟」的爭用，共媒體無從放大 |

- **一句話**：**共媒體的傷害只在「有人跟 client 搶同一顆碟」時爆**——backfill/recovery、高並行寫這種製造爭用的情境最明顯；穩態單 VM 天花板（E-02）與本來就不缺並行的參數（E-22），換不換媒體都差不多。
- **§6 參數建議全數維持**：queue_depth=256 對高並行純加分且 p99 更好（clean 下寫側加成更大）、shards=8 已足、虛擬化稅集中高並行讀、writeback 危險（未重跑，機制與媒體拓樸無關）。

### 9.3 侷限
- **E-30 資料量**：clean 下每 OSD 實資料 ~8G，backfill 快、尖峰窗短；生產 TB 級/OSD 的 backfill 歷時更久，尖峰次數會更多（機制可移植、絕對尖峰**數**不可移植）。
- **E-19 生效驗證缺口**：host `config_info`（讀 queue_depth=N）因 NSG 擋 worker internal IP 未能執行，改以行為級 A/B 差異證明生效（qd64 與 qd256 產出顯著不同、否則應相同）；此外 rw-qd32×4 qd256=36.8k 部分受 osd.2/osd.4 的 mclock outlier 配額墊高，絕對倍率含此環境噪音。
- **未重跑項**：writeback / CPU limit / failover 等機制級結論本次未重跑——它們與 OSD↔媒體拓樸無關，無重跑必要。
