#!/bin/bash
# E-12 io mode 對照 orchestrator（A=留空/native, th=threads）
# 交錯 N=1..3 × [A, th]；bash experiments/kubevirt-rbd-tuning/tools/e12-orchestrator.sh <bundle-dir>
...
apply_variant(){ # $1 = A | wt | wb
  case $1 in
    A)  cpx "kubectl patch vm -n vmtest baseline --type=json -p '[{\"op\":\"remove\",\"path\":\"/spec/template/spec/domain/devices/disks/1/io\"}]'" >/dev/null 2>&1 || true ;;
    th) cpx "kubectl patch vm -n vmtest baseline --type=json -p '[{\"op\":\"add\",\"path\":\"/spec/template/spec/domain/devices/disks/1/io\",\"value\":\"threads\"}]'" >/dev/null ;;
  esac
...
verify_variant(){ # $1 = A | wt | wb ；斷言 /dev/data 的 direct 與 ua-data 的 write-cache
  ...
  echo "$node" | grep -q "\"aio\":\"$want_aio\"" || { log "FATAL verify $1: aio!=$want_aio: $node"; exit 1; }
  log "verify $1 ok (aio=$want_aio)"
}
