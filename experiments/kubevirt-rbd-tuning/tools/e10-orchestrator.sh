#!/bin/bash
# E-10 cache mode 三方對照 orchestrator（A=留空/none, wt=writethrough, wb=writeback）
# 交錯順序：N=1..3 × [A, wt, wb]，每次切換 = patch + stop/start + 生效驗證。
# 從 repo root 執行：bash experiments/kubevirt-rbd-tuning/tools/e10-orchestrator.sh <bundle-dir>
...
apply_variant(){ # $1 = A | wt | wb
  case $1 in
    A)  cpx "kubectl patch vm -n vmtest baseline --type=json -p '[{\"op\":\"remove\",\"path\":\"/spec/template/spec/domain/devices/disks/1/cache\"}]'" >/dev/null 2>&1 || true ;;
    wt) cpx "kubectl patch vm -n vmtest baseline --type=json -p '[{\"op\":\"add\",\"path\":\"/spec/template/spec/domain/devices/disks/1/cache\",\"value\":\"writethrough\"}]'" >/dev/null ;;
    wb) cpx "kubectl patch vm -n vmtest baseline --type=json -p '[{\"op\":\"add\",\"path\":\"/spec/template/spec/domain/devices/disks/1/cache\",\"value\":\"writeback\"}]'" >/dev/null ;;
  esac
...
verify_variant(){ # $1 = A | wt | wb ；斷言 /dev/data 的 direct 與 ua-data 的 write-cache
  ...
  case $1 in
    A)  want_direct=true;  want_wc=on ;;
    wt) want_direct=false; want_wc=off ;;
    wb) want_direct=false; want_wc=on ;;
  esac
  echo "$node" | grep -q "\"direct\":$want_direct" || { log "FATAL verify $1: direct!=$want_direct: $node"; exit 1; }
  echo "$dev"  | grep -q "\"write-cache\":\"$want_wc\"" || { log "FATAL verify $1: write-cache!=$want_wc: $dev"; exit 1; }
  log "verify $1 ok (direct=$want_direct write-cache=$want_wc)"
}
