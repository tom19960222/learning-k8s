...
1. **失聯恢復**：與 NTP server 失聯 1hr / 4hr / 24hr，時鐘頻率誤差 0 / ±10 / ±100 / ±1000 ppm 的組合下，恢復連線後多久誤差降到 50ms 以下？
...
| kernel 滑動修正上限 `MAXFREQ` | 500000 ns/s = 500 ppm | `linux/include/linux/timex.h:136` |
...
3 時長 × 7 頻率誤差（0、±10、±100、±1000 ppm），折疊後初始 offset：

| | 0ppm | ±10ppm | ±100ppm | ±1000ppm |
|---|---|---|---|---|
| 1hr | 0（對照） | ±36ms | ±360ms | ±3.6s |
| 4hr | 0 | ±144ms | ±1.44s | ±14.4s |
| 24hr | 0 | ±864ms | ±8.64s | ±86.4s |

Source 推導預測（文章先寫預測、結果跑完回填）：

- `|offset| < 50ms`：立即合格，收斂 ≈ 0
- 50–400ms：kernel slew ≤ 500ppm → 360ms 約 10 分鐘
- `> 400ms`：timesyncd 首個 sample 直接跳錶，秒級
- ±1000ppm：超過 `MAXFREQ` 補償上限，預測**永不穩定收斂**（淨漂移 500ppm；poll 拉長後跨 poll 漂移 > 0.4s 又觸發跳錶）。timeout 30 分鐘記 non-convergent——這本身是 finding。

### 實驗 2：399ms 滑動修正（2 cells）

399ms（門檻下 1ms → 純 slew，預測 `(399-50)/0.5ms·s⁻¹ ≈ 700s`）+ **401ms 對照**（門檻上 1ms → 跳錶，秒級），一組對照把 0.4s 懸崖打在圖上。

### 實驗 3：重啟對時（4 cells）

收斂狀態注入 offset {10, 50, 100, 500ms} → `systemctl restart systemd-timesyncd` → 量測 (a) restart 到第一次 clock 調整的延遲（journal + probe 交叉，回答「是否馬上對時」）、(b) 到 <50ms 時間。預測：10ms 無感、50ms 邊界、100ms slew ~100s、500ms 跳錶秒級。
...
預估 wall-clock：校準 ~1h、實驗 1 約 2–3h、實驗 2 ~0.5h、實驗 3 ~1h、實驗 4 預設 ~10h（quick 4h）。全程可無人值守。
